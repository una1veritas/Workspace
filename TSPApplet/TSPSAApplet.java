...
public class TSPSAApplet extends Applet 
{
	...
	public void init()
	{
		...
		add(label2);
		label2.setBounds(204,264,72,24);
		add(label3);
		label3.setBounds(50,264,56,24);
		try {
		...
		label4.setText("Tour length:");
		add(label4);
		label4.setBounds(132,264,70,24);
		label5.setText("Temp.:");
		add(label5);
		label5.setBounds(12,264,38,24);
		//}}
		add(canvas1);
	...
	int [] solution = null;
	double temp;
	final double modratio = (1 - Math.exp(- Math.pow((double)tsp.size(),0.5)) );
	...
	void button1_ActionPerformed(java.awt.event.ActionEvent event)
	{
		...
		double currCost, newCost;
    	int [] newSolution;
		
		if (solution == null) {
		    return;
		}
		tsp.tryModifiedNeighbor(solution, temp);
		// update temp
		//temp = temp*modratio;
        /*
	    ...
	    //}
	    //System.out.println();
        label2.setText(String.valueOf(  (double)((int)(tsp.tourLength(solution)*100))/100 ));
        label3.setText(String.valueOf(temp));
	    repaint();
	}
	...
	class SearchThread extends Thread {
	    public void run() {
    	    ...
    	    while (temp > 1) {
    	        if (tsp.tryModifiedNeighbor(solution, temp) ) {
    	            temp = temp*modratio;
    	            failedCount = 0;   	            
    	        } else {
    	            failedCount++;
    	            if (failedCount > (tsp.size()*tsp.size()) ) {
    	                if (!tsp.findBetterNeighbor(solution)) {
                           break;
                        }
                        temp = temp*modratio;
