...
/* クラス */
typedef struct ClassedLectures {
	int id;
	int num;   /* 属する講義の数 */
...
void read_and_ready(Config *config, Weight *weight, Num *num, 
					Lecture **lct, ClassedLectures **cls, /*Bilingual **blg,*/ 
					Teacher **tch, /*int *nrgl_lects,*/ int **period_id, int **day_id, int **day,
...
/*インスタンスを表示*/
void printdata(FILE *fpo, Num num, Lecture *lct, ClassedLectures *cls, /*Bilingual *blg,*/ Teacher *tch/*, int nrgl_lects*/);
...
/* 初期解作成 */
void initsol(Num num, Lecture *lct, Teacher *tch, ClassedLectures *cls, Timetable f);
...
/* 解の制約違反を種類別に数えて保持 */
int count_offence(Num num, Lecture *lct, ClassedLectures *cls, Teacher *tch, Timetable f, Offence *offence);
...
int valuate(
			...
			Lecture *lct, ClassedLectures *cls, /*Bilingual *blg,*/ Teacher *tch, /*int nrgl_lects,*/ int *day,
...
/* 解の評価値を要望別に表示 */
void printval(FILE *fpo, Weight weight, Num num,
			  Lecture *lct, ClassedLectures *cls, /*Bilingual *blg,*/ Teacher *tch, /*int nrgl_lects, */
			  Timetable f, Penalty penalty);

/* 局所探索 */
int local_search(Config config, Weight weight, Num num,
				 Lecture *lct, ClassedLectures *cls, /*Bilingual *blg,*/ Teacher *tch, /*int nrgl_lects,*/
				 int *day, 
				 Timetable f, Timetable new_f, Offence *offence, Penalty *penalty, 
				 Utility *utility);
...
void make_moved_sol(
					Num num, Lecture *lct, ClassedLectures *cls, Teacher *tch, 
					int cls1, int time1, int cls2, int time2, Timetable f, Timetable new_f, Utility utility);

/* 解fに近傍操作move(cls1, time1, cls2, time2)を加える */
void operate_move(Num num, Lecture *lct, ClassedLectures *cls, Teacher *tch, int cls1, int time1, int cls2, int time2, Timetable f, Utility utility);
...
void output_timetable(FILE **fpo2, Num num, Lecture *lct, ClassedLectures *cls, Teacher *tch, int *period_id, int *day_id, Timetable f);

void output_best_timetables(Config config, Num num, Lecture *lct, ClassedLectures *cls, Teacher *tch, int *period_id, int *day_id, Timetable *best_sol);
...
int main() {
	...
	read_and_ready(&config, &weight, &num, 
				   &lct, &cls, /*&blg,*/ &tch, /*&nrgl_lects,*/ &period_id, &day_id, &day, 
				   &f, &new_f, &penalty, &utility,
				   &best_sol, &best_off, &best_val);
	
	fpo = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/timetables/result", "w");
	printconfig(fpo, config);
	//	printconfig(stderr, config);
	printweight(fpo, weight);
	printdata(fpo, num, lct, cls, /*blg,*/ tch/*, nrgl_lects*/);
	
	fprintf(stderr,"Into mode %d...\n", config.MODE);
	
	if (config.MODE == 0) {
		for (i = 0; i < config.WIDTH; i++) {
			initsol(num, lct, tch, cls, f);
			fprintf(stderr, "initsol finished...\n");
		...
		for (i=0; i<config.SAVE; i++)
			if (best_off[i] >= 0)
			{
				...
				//fpo2 = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/timetables/csv_timetable", "w");
	...
	{
		fpi4 = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/sol","r");
		readsol(fpi4, num, f);
		fclose(fpi4);
...
//	int row_p;
	...
	//	*fpi3 = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/numbers", "r");
	fpi5 = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/task_relation", "r");
	fpi6 = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/processor_relation", "r");
	
	fpi1 = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/config", "r");
	fscanf(fpi1, "%d %d %d %d %d", &(config->MODE), &(config->WIDTH), &(config->DEPTH), &(config->SAVE), &(config->SEED));
	srand(config->SEED);
	fclose(fpi1);
	
	fpi7 = fopen("/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/period_relation", "r");
	...
	*day_id = (int *)malloc(sizeof(int) * (*num).time);
	...
	//メモリ確保，データ格納
	*cls = (ClassedLectures *)malloc(sizeof(ClassedLectures) * (*num).clsnum);
	...
	*tch = (Teacher *)malloc(sizeof(Teacher) * (*num).tchnum);
	...
	// Setting penalty weights from db.
	 association assoc[] = 
	 {{"PREFER", 0},
	 {"R_WEEKOVER", 0}, 
	 {"R_WEEKUNDER", 0}, 
	 ...
	 if ( read_from_db("daisygw.daisy.ai.kyutech.ac.jp","yukiko","nishimura","schedule",
					   "name", "weight", "penalties", assoc) ) {
		 weight->PREFER = 5;
		 weight->R_WEEKOVER = 300;
		 weight->R_WEEKUNDER = 500;
		 weight->R_DAYSOVER = 10;
		 weight->R_DAYSUNDER = 10;
		 weight->R_HOLE = 5;
		 weight->R_DESIRE = 2;
		 weight->N_WEEKOVER = 400;
		 weight->N_WEEKUNDER = 200;
		 weight->N_DAYSOVER = 50;
		 weight->N_DAYSUNDER = 50;
		 weight->N_HOLE = 25;
		 weight->N_DESIRE = 1;	 
	 } else {
...
fprintf(fpo, "\n");*/
	...
	for (i=0; i<num.lctnum; i++)    
    {
		fprintf(fpo, "#%-2d  %2d    %2d   ", i, lct[i].classedTo, lct[i].qualification);
	...
	fprintf(fpo, "\n");
			   ...
			   void printsol(FILE *fpo, Num num, Lecture *lct, Teacher *tch, int *day, Timetable f)
			   {
				   ...
				   for (i=0; i<num.lctnum; i++)
				   {
					   fprintf(fpo, "#%-2d: %2d   %3d   ", i, lct[i].classedTo, lct[i].qualification);
					   //     fprintf(fpo, "#%-2d  %2d   %4d %-10s: ", i, lct[i].classedTo, lct[i].qualification, lct[i].name);
					   tmp = -1;
					   for (j=0; j<num.time; j++)
					   {
						   check = 0;
			   ...
			   void initsol(Num num, Lecture *lct, Teacher *tch, ClassedLectures *cls, Timetable f)
			   {
				   ...
				   while (rest)
				   {
					   ...
					   rand_time = r;
					   rand_cls = lct[rand_lct].classedTo;
...
/* 局所探索 */
int local_search(Config config, Weight weight, Num num,
				 Lecture *lct, ClassedLectures *cls, /*Bilingual *blg,*/ Teacher *tch, 
				 /*int nrgl_lects,*/ int *day, 
				 Timetable f, Timetable new_f, Offence *offence, Penalty *penalty, 
				 Utility *utility) {
	int i; //, j;
	...
	{
		count_offence(num, lct, cls, tch, f, offence);
		fprintf(stderr, "offence: %d,  ", (*offence).total);
		valuate(weight, num, lct, cls, /*blg,*/ tch, /*nrgl_lects,*/ day, f, penalty);
		fprintf(stderr, "penalty total: %d\n", (*penalty).total);
			...
			for (k=0; k<num.tchnum; k++)
				if (f[i][j][k])
				{
					fprintf(*fpo2, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
							cls[lct[i].classedTo].id, period_id[j], tch[k].id, cls[lct[i].classedTo].qualification_id, cls[lct[i].classedTo].num, cls[lct[i].classedTo].num,
...
void output_best_timetables(Config config, Num num, Lecture *lct, ClassedLectures *cls, Teacher *tch, int *period_id, int *day_id, Timetable *best_sol)
{
	...
	for (i=0; i<config.SAVE; i++)
	{
		strcpy(filename, "/Volumes/WORKSPACE06/Development/SchoolTimeTabling/a/timetables/timetable");
		len = strlen(filename);
		n = 0;
				...
				for (c=0; c<num.tchnum; c++)
					if (best_sol[i][a][b][c])
					{
						fprintf(fp, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
								cls[lct[a].classedTo].id, period_id[b], tch[c].id, cls[lct[a].classedTo].qualification_id, cls[lct[a].classedTo].num, cls[lct[a].classedTo].num,
