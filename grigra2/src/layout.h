...
class GridLayout {
	...
	int32 checkPointPair(Point &p, Point &q);int32 prevX(int32 ix, int32 iy,
			PointSequence &x, PointSequence &y);int32 prevY(int32 ix, int32 iy,
			PointSequence &x, PointSequence &y);
	void listCountRects(PointSet &set, PointSequence &sortx,
			PointSequence &sorty);
	void calcTranslateTable(PointSet &set, PointSequence &x,
			PointSequence &y, grid<int32> &cr);

	void applyLayout(PointSet &set, PointSequence &x, PointSequence &y,
			grid<int32> &cr, grid<Point> &tt, grid<Point> &sd);

public:
	GridLayout(int32 grid_size) :
			grid_size(grid_size) {
	}
	~GridLayout() {
	}
