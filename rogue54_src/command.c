...
void rs_write_int(FILE *, int);
void rs_write_string(FILE *, const char *);
...
/* {{ */
		when CTRL('V'):
		    ...
		    {
			rs_write_int(fptr, terse);
			rs_write_int(fptr, fight_flush);
			rs_write_int(fptr, jump);
			rs_write_int(fptr, see_floor);
			rs_write_int(fptr, passgo);
			rs_write_int(fptr, tombstone);
			rs_write_int(fptr, inv_type);
			if (whoami[0] == ' ' || whoami[0] == '!')
			    whoami[0] = '\0';
			rs_write_string(fptr, whoami);
			rs_write_string(fptr, fruit);
			fclose(fptr);
