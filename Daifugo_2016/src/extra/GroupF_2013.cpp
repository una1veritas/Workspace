...
#include "CardSet.h"
#include "Player.h"
#include "GroupF_2013.h"


bool GroupF_2013::approve(const GameStatus & gstat) {
...
bool GroupF_2013::follow(const CardSet & pile, CardSet & s) {
  Card tmp;
  int n;
  //int pairs;
...
void GroupF_2013::cardSetOfSameRanks(CardSet & s, int rank) {
  Card tmp;
...
void GroupF_2013::sort(CardSet & s) {
  Card tmp;
...
int GroupF_2013::rankTostren(int rank){
  int stren;
  switch(rank)
    {
    case 0:
...
int GroupF_2013::strenTorank(int stren){
  int rank;
  switch(stren)
    {
    case 0://rank3
...
void GroupF_2013::setstren(int stren[],CardSet s){
  int i;
  for(i = 0; i < 14; i++)
    stren[i] = 0;
  for(i = 0; i < s.size(); i++)
...
int GroupF_2013::seekstren(int rank, int size,int strength[]){
...
int GroupF_2013::SelectNextNum(CardSet & pile,int strength[]) {
...
void GroupF_2013::printstren(int stren[]){
  int i;
  std::cout << "3| ";
  for(i=0;i<5;i++)
    std::cout << stren[i] << " ";
...
int GroupF_2013::findpair(int size, int strength[]){
  int ret=0;
  for(int i=0;i<14;i++)
    if(strength[i] == size)
      ret++;
...
int GroupF_2013::pairsnum(int strength[]){
  int ret=0;
  for(int i=1;i<=4;i++)
...
int GroupF_2013::havecards(int rank,int strength[]){
  return strength[rankTostren(rank)];
}

int GroupF_2013::SelectNum(int num) {
  if(num>=0){
    for(int i=0;i<inHand().size();i++){
      if(inHand().at(i).getRank() == num){
	//	std::cout << "success" << std::endl;
...
int GroupF_2013::first_half(CardSet pile,int strength[]){
  int ret = 0;
  if(pile.isEmpty()){
    for(int i=0;i<14;i++)
...
int GroupF_2013::latter_half(CardSet pile,int strength[]){
  int ret = 0;
  if(pile.isEmpty()){
    for(int i=0;i<14;i++)
...
int GroupF_2013::getRank(const CardSet & cset) {
	if ( cset.isEmpty() )
		return 0;
