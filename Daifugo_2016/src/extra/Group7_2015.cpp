...
#include "Card.h"
#include "CardSet.h"
#include "Group7_2015.h"
#include "Player.h"

#define TAIKAI_HONBAN

void Group7_2015::ready() {
	memory.makeEmpty(); //memory.clear();
	trump.makeEmpty(); //trump.clear();
}

//使用されたカードをmemory記憶する
bool Group7_2015::approve(const GameStatus & gstat){
...
bool Group7_2015::follow(const GameStatus & gstat, CardSet & s) {
	CardSet pile(gstat.pile);
	Card tmp, tmp2;
 ...
 */
bool Group7_2015::cardLessThan(const Card & c1, const Card & c2) {
 ...
 */
bool Group7_2015::cardEquals(const Card & c1, const Card & c2) {
 ...
 * 枚数は少ないので，効率は気にしない．
 */
void Group7_2015::sortInHand(void) {
...
void Group7_2015::memorysort(void) {
...
int Group7_2015::cardNum(int num, CardSet tmp){
	int c = 0;
...
int Group7_2015::checkNum(int num){
...
bool Group7_2015::checkSt(int num){
...
int Group7_2015::getRandom(int min, int max){
  	return min + (int)(rand()*(max-min+1.0)/(1.0 + RAND_MAX));
}

//上のやつの2枚出しの時で調べるバージョン
//jokerが出てない場合は調べることが出来ない
bool Group7_2015::checkSt2(int num){
	if(!memory[memory.size()-1].isJoker() && !hand[hand.size()-1].isJoker())
		return false;
  	else if(hand[hand.size()-1].isJoker())
    	return true;
