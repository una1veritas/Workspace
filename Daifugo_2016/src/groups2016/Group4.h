...
class Group4 : public Player {
    CardSet mymemory;
    int numOfEachRank[14];
    int pileOfEachRank[14];
    int pilePerson;     //カードを出した人を記録
    
public:
    Group4(const char * name = "Test People") : Player(name) {
    ...
    //自作関数
    int numExitCards(const GameStatus & gstat);
    
    void cardSetOfSameRanks(CardSet &hand);
    //void pileOfSameRanks(CardSet &pile);
    int mostCardStrength();
    CardSet & lastTwoCards(const CardSet & cs, CardSet & mycs);
};
