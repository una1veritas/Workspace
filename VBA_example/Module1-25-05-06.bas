...
Public Function EnrolledLast4Semesters(srecs As Variant, semeslist As Variant)
    ...
    ReDim Semesters(1 To UBound(semeslist)) '‰¼‚É”÷–­‚É‘å‚«‚ß‚É
    Dim bgix As Integer
    ...
    For ix = 1 To UBound(srecs)
        ...
        evtstart = srecs(ix, 2) '“ú•t
        evtend = srecs(ix, 3)   '“ú•t
        evttype = srecs(ix, 4)  'ˆÙ“®“à—e
        If evttype = "“üŠw" Then
            onSchool = True
            lastPrevEvent = evttype
            lastPrevDate = evtstart
            bgix = ix
        ElseIf evttype = "•œŠw" Then
            onSchool = True
            lastPrevEvent = evttype
            lastPrevDate = evtstart
        ElseIf evttype = "‹xŠw" Or evttype = "‘ŞŠw" Or evttype = "İŠw’†" Then
            If onSchool = True Then
                If evttype = "‘ŞŠw" Then
                    evtstart = evtstart + 1
                ElseIf evttype = "İŠw’†" Then
                ...
                If Month(lastPrevDate) <> 4 And Month(lastPrevDate) <> 10 Then
                    MsgBox "ŠwŠúŠJn‚ª‚SŒ‚Å‚à‚P‚OŒ‚Å‚à‚È‚¢‚¼ƒGƒ‰["
                    Debug.Print lastPrevDate, lastPrevEvent
                    Stop
                End If
                If Day(lastPrevDate) <> 1 Then
                    MsgBox "ŠwŠúŠJn‚ª‚P“ú‚Å‚È‚¢‚¼ƒGƒ‰["
                    Debug.Print lastPrevDate, lastPrevEvent
                    Stop
                End If
                ...
                Next mx
            Else
                MsgBox "Error: ‚Í‚¶‚Ü‚Á‚Ä‚¢‚È‚¢İŠwŠúŠÔ‚ª‚ ‚é‚æB‘åä•vHƒGƒ‰["
            End If
            EnrollFlag = False
        End If
    Next ix
