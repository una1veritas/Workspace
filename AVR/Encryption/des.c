...
BOOTFLASH unsigned char sTable[256] = {
  // S-box 1
...
BOOTFLASH unsigned char sOrder[5] = {0, 5, 1, 2, 3};
...
BOOTFLASH unsigned char eTable[48] =
{
	31, 0,  1,  2,  3,  4,
	3,  4,  5,  6,  7,  8,
	7,  8,  9,  10, 11, 12,
...
void xorFlash(unsigned char *left, unsigned char BOOTFLASH *right)
{
...
void s(unsigned char *out, unsigned char *in)
{
	...
	unsigned char BOOTFLASH *sBox = sTable;
	unsigned char bytes;
	...
	do
	{
		...
		unsigned char assemblyByte;
		
		nibbles = 2;		
		do
		{
			unsigned char sboxByte;
			unsigned char BOOTFLASH *order;
			unsigned char index;
			unsigned char bit;
...
void desDecrypt(unsigned char *buffer, unsigned char *chainBlock)
{
	...
	{    
		...
		xorFlash(temp1, &kTable[6 * (x & DES_MASK)]);
		s(temp2, temp1);
...
void permute2(unsigned char size, unsigned char *out,
...
//				throw new CreateException(ERROR_KEY_PARITY, keystr);
		...
		permute2(56, tempKey, key[i].data(), pc1Table);
		for (z = 0; z < 16; z++)
		{
