...
class Mining {
	...
	public Mining(Vector strs, double confidence, double support) {
		...
		System.out.println("Finished Making SuffixTree");
  ...
  private void makeList(Vector strs, SuffixTree tree, double confidence, double support) {
    ...
    int match;  // A ... B
    int node1_count, node2_count;
    
    for ( enum1 = tree.enumeration(); enum1.hasMoreNodes(); ) {
      node1 = (SuffixTree) enum1.nextElement();
      if ( (node1_count = node1.count()) < strs.size() * support ) {
	enum1.skipChildren();
	continue;
      }
      for ( enum2 = tree.enumeration(); enum2.hasMoreNodes(); ) {
	//System.out.println();
	node2 = (SuffixTree) enum2.nextElement();
	if ( (node2_count = node2.count()) < strs.size() * support ) {
	  enum2.skipChildren();
	  continue;
	}
	match = compare(strs, node1.representingString(), node2.representingString());
	if ( (match < node1_count * confidence) || (match < strs.size() * support) ) { 
	  enum2.skipChildren();
	  continue;
	}
	if ( (node1.representingString().length() != 0) 
	     && (node2.representingString().length() != 0) ) {
	  answer.addElement(new MiningInfo(node1.representingString(), node2.representingString(), match , node1_count, strs.size()));
	  System.out.println("<" + node1.representingString() +"..."+ node2.representingString() + ">" 
			     // +" \t pattern matched "+ match + ", pre matched " + node1_count + ", & rule holds " + match + " among " + strs.size());
			     +" \t"+ (100*match/node1_count) + " \t" + (100*match/strs.size()));
	}
      }
    }
  }
