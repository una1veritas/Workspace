...
public class TimeTableScheduling {
	...
	public void storeTimeTableOnDBTable(String tbl, int project_id, String tstamp ) throws Exception {
		...
		setStatementValue(5, timetable.toStoreString());
	...
	private Timetable readTimetableFromDBTable(int project_id) throws Exception{
		...
		HashMap lectures = new HashMap();
		ArrayList periods = new ArrayList();
		
		HashMap availabilities = new HashMap();
		HashMap qualifications = new HashMap();
		HashMap opportunities = new HashMap();
		String sql;
		...
		for (Iterator i = teachers.keySet().iterator(); i.hasNext(); ) {
			Processor teacher = (Processor) i.next();
			availabilities.put(teacher, new HashSet());
			qualifications.put(teacher, new HashSet());
			...
			while(results().next()){
				...
				((HashSet) availabilities.get(teacher)).add(ppp);
			}
			...
			while(results().next()){
				...
				((HashSet) qualifications.get(teacher)).add(qid);
			}
		...
		//task_idが開講可能な時間を取得
		for (Iterator i = lectures.keySet().iterator(); i.hasNext(); ) {
			Task l = (Task) i.next();
			opportunities.put(l, new HashSet());
			...
			while(results().next()){
				...
				((HashSet) opportunities.get(l)).add(pd);
			}
		}
	...
	public void reportTo(PrintWriter pw) throws Exception{
		...
		for (Iterator i = rules.iterator(); i.hasNext(); ){
			t = (SQLRule) i.next();
			int num = 0;
			for (Iterator s = t.queries().iterator(); s.hasNext(); ) {
				if ( !execute((String) s.next()) )
					continue;
				if (results().next()) {
