...
void __attribute__((picinterrupt(("high_priority")))) High_Priority_ISR(void);
void UART_WriteChar(uint8_t data);
uint8_t UART_ReadChar(void);
_Bool UART_RxBufferDataReady(void);
uint16_t UART_RxBufferCount(void);
_Bool UART_TxBufferFreeSpace(void);
...
void __attribute__((picinterrupt(("high_priority")))) High_Priority_ISR(void) {
    ...
    if (PIR1bits.TXIF && PIE1bits.TXIE) {
        if (tx_buffer_head != tx_buffer_tail) {
            ...
            tx_buffer_tail = (tx_buffer_tail + 1) % 64;
        } else {
...
uint16_t UART_RxBufferCount(void) {
    if (rx_buffer_head >= rx_buffer_tail) {
        ...
        return (64 - rx_buffer_tail + rx_buffer_head);
    }
}
...
uint8_t UART_ReadChar(void) {
    uint8_t data = 0;
    while (!UART_RxBufferDataReady());


    INTCONbits.GIEH = 0;
    data = rx_buffer[rx_buffer_tail];
    rx_buffer_tail = (rx_buffer_tail + 1) % 64;
    INTCONbits.GIEH = 1;
...
_Bool UART_TxBufferFreeSpace(void) {
    uint16_t next_head = (tx_buffer_head + 1) % 64;
    return (next_head != tx_buffer_tail);
}


void UART_WriteChar(uint8_t data) {
    while (!UART_TxBufferFreeSpace());


    INTCONbits.GIEH = 0;
    tx_buffer[tx_buffer_head] = data;
    tx_buffer_head = (tx_buffer_head + 1) % 64;
    PIE1bits.TXIE = 1;
    INTCONbits.GIEH = 1;
}
