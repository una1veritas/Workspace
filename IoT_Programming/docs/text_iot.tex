\documentclass[11pt,a4,epsf]{article}
...
この入門ではインタラクティブモードを使うため，必要になる都度ライブラリのインポートをすることとにして，先にすすもう．

\subsection{変数の宣言，代入，演算}

Python では，{\bf 変数の宣言は不要}であるが，値を定義済みでない変数の値を参照しようとするとエラーとなる．
間違いが起きないように，変数に初期値を代入する式を書いておき，宣言のかわりにするとよい．

変数への代入などの算術式，論理式などは C 言語とほぼかわらない．
ただし，論理 AND \verb+&&+, OR \verb+||+ は \verb+and+, \verb+or+ と書く．
