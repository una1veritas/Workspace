...
/* {{ */
int rs_write(FILE *, void *, size_t);
int rs_write_boolean(FILE *, bool);
...
/* {{ */
		    when CTRL('V'):
			if ((fptr = fopen(cfgfile, "w")) != NULL)
			{
			    rs_write_boolean(fptr, terse);
			    rs_write_boolean(fptr, fight_flush);
			    rs_write_boolean(fptr, jump);
			    rs_write_boolean(fptr, slow_invent);
			    rs_write_boolean(fptr, askme);
			    rs_write_boolean(fptr, auto_pickup);
			    rs_write_boolean(fptr, menu_overlay);
			    if (whoami[0] == ' ' || whoami[0] == '!')
				whoami[0] = '\0';
			    rs_write(fptr, whoami, NAMELEN);
			    fclose(fptr);
			    msg("You save the current options in a file");
			}
			after = FALSE;
/* }} */
	    when '+':	/* instant karma! */
