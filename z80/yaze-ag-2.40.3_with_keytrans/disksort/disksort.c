...
sort_daten()       /* alle Daten der disk sortieren (dauert etwas ... ) */
{
   ...
   if (modus) printf("Sort the data blocks (please wait) ...\n");
   while(i=get_block())
      {
         comm_operation(i,&buffer,&ex_buf);
...
printf("DISKSORT.C  (c) Copyright 1988 by Michael Schewe  3119 Gollern Nr.17\n");
    ...
    */
   if (modus) printf ("free working memory before starting: %u bytes\n",space());
    {
    ...
    */
   if (modus)  printf("Minimum free working memory: %u bytes\n",space() );
   cfree( baum_array );
   ...
   if (modus) printf ("Free working memory at the End: %u bytes\n",space());
   check_crc();
   error("alles ok");
}
