...
read_dir()
{
   ...
   if (modus) printf("reading directory ...\n");
   ...
   if (modus) printf("Timestamps %s present.\n",timestamps ? "are":"are not");
   ...
   if (modus)
    {
       ...
       */
      printf ("%d dir blocks needs %u bytes\n",dirbloecke,bytes);
      printf("Sort entries in a binary tree ...\n");
    }
...
write_dir()
{
   ...
   if (modus)
	 ...
	 * printf("Versuche\nzu Unterlassen !!!\n");
	 */
	printf("Write new directory. NOW don't try to abort the following ");
	printf("sort process !!!\n");
    }
