...
crc_check()
{
  ...
  if (modus) printf("Create the CRC-File CRC.SRT (please wait) ...\n");
...
check_crc()            /* CRC's am Ende noch einmal vergleichen ... */
{
   ...
   if (modus) printf("Compare the CRC's with the files (please wait)...\n");
   reset();
   buffer=my_alloc(blocksize);
 ...
 if (modus&&(modus<3))
   ...
   printf("All files have exactly the same data as bevor sorting.\n");
}
