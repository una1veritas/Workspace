...
public class TimeTableSchedulingMain {
	public static void main(String args[]) throws Exception{
		...
		if(mode==4){
			tts = new TimeTableScheduling(host,db,user,pass,file,readType,project_id);
		}else{
			tts = new TimeTableScheduling(host,db,user,pass,file,readType);
		}
		...
		for(int i = 1; i<=count; i++){
			...
			if(count_offence < t_count_offence){
				int index = list.indexOf(new SaveTimeTable(null,t_count_offence,t_va