...
int main(int argc, char * argv[]) {
	...
	cout << "Julian Day number = "<< integerPart(jd) << "." << integerPart(jd*10)%10 << integerPart(jd*100)%10 << endl;

	cout << "Day of the week = " << wd[integerPart(integerPart(jd)+1.5) %7] << "." << endl;
	double cal = CalendarDate((double)2456205.00);
...
double JulianDay(int y, int m, double dd) {
	...
	return int(365.25 * y) + int(30.6001 * (m+1)) + dd + b + 1720994.5;
}
