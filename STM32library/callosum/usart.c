...
Ring rx, tx;
USARTPort usart3 = { &rx, &tx, USART3 };

void begin(USARTPort * port, uint32_t baud) {
...
//	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, (FunctionalState) ENABLE);
	...
	USART_Init(port->USARTx, &USART_InitStruct); // again all the properties are passed to the USART_Init function which takes care of all the bit setting

	USART_ITConfig(port->USARTx, USART_IT_RXNE, (FunctionalState) ENABLE); // enable the USART3 receive interrupt
	USART_ITConfig(port->USARTx, USART_IT_TXE, (FunctionalState) DISABLE);
/*
USART_IT	説明
...
*/
	...
	USART_Cmd(port->USARTx, (FunctionalState) ENABLE);
...
uint16_t write(USARTPort * port, const uint16_t ch) {
	while (!(port->USARTx->SR & 0x00000040))
	;
//	ch = usart3tx.ringout();
	USART_SendData(port->USARTx, (uint16_t) ch);
	// Loop until the end of transmission
	while (USART_GetFlagStatus(port->USARTx, USART_FLAG_TC )
			== RESET) {
	}
	return 1;
//	return buffered_write(ch);
	...
	while (USART_GetFlagStatus(port->USARTx, USART_FLAG_TC )
				== RESET);
	ringin(port->tx, ch);
	USART_ITConfig(port->USARTx, USART_IT_TXE, (FunctionalState) ENABLE);
	return 1;
	*/
}
...
uint16_t print(USARTPort * port, const char * s) {
	uint16_t n = 0;
	while ( *s ) {
		write(port, *s);
		s++;
...
/*d
uint16_t print_uint32(uint32_t val, const uint8_t base) {
	uint16_t n = 0;
	bool msd = false;
...
uint16_t print_float(float val, uint8_t prec) {
	uint16_t n = 0;
	if ( val < 0 ) {
		write('-');
		val = -val;
