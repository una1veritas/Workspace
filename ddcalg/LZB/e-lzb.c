...
void put_code(int h, int p, int lng, int part)
{				/* LZBフォーマットの2値出力 */
    bit_out(fp_out, h);
    if (h == 0)
	fit_code(p, 256);
...
int buf_point = 0, buf_leng = 0;/* buf_point 配列 buf[] の絶対位置 */
...
int search(int pos0, int pos1, int *lng)
{				/* 一致記号数探索 */
...
int set_tree(int pos, int *match_pos, int *match_leng)
{				/* 4文字列を2分木に挿入 */
