...
#define c 10
#define k  (long) fabs(log(2*8*c*Maxpoint/e)/log(2))
#define L (1L << k)
        ...
        //next_edgeでないほうのedgeをnext_edgeにする．
         ...
         j=discone[i].place-1;
        	...
        	for(t=0;t<= quad[quad[quad_num].place[j+1]].point_num-1;t++){
                if(discone[i]==answers[j].p_cone[t].line && 0==serach_same_point(total_line,answers[j].p_cone[t].c_point(),ans)){//&& (j+1)==discone[i].place){
                point_num++;
