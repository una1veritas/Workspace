...
public class MasterHandler extends Thread {
  ...
  private MasterHandlerMethods handlerMethods;
  
  public MasterHandler(Socket socket, CommonInfo commonInfo) {
    this.socket = socket;
    ...
    handlerMethods = new MasterHandlerMethods(commonInfo, socket, 
...
/*
	...
	} else if ((commandType.equals("STAFFNAME")) && (serviceName.equals("STAFF"))) {
	  handlerMethods.sendStaffName(paramValues);
	} else if (commandType.equals("INITSTAFFPASSWD")) {
	  if (commandCode.equals("INITPASSWD")) {
	    handlerMethods.initializeAndSendStaffInitialPassword(paramValues); 
	  } else if (commandCode.equals("REGISTRATE")) {
	    handlerMethods.registrateAndSendStaffInitialPassword(paramValues);
	  } 
	...
	} else if (commandType.equals("UPDATECALENDAR")) {
	  handlerMethods.updateCalendar(serviceName, commandCode, paramValues); 
	} else if (commandType.equals("JIKANWARICONTROL")) {
	  handlerMethods.jikanwariControl(commandCode, paramValues); 
	} else if (commandType.equals("COPYCURRICULUMTONEXTYEAR")) {
	  handlerMethods.copyCurriculumToNextYear(paramValues);
	} else if (commandType.equals("COPYEDUCURRICULUMTONEXTYEAR")) {
	  handlerMethods.copyEduCurriculumToNextYear(paramValues);
	} else if (commandType.equals("COPYCLASSINFOTONEXTYEAR")) {
	  handlerMethods.copyClassInfoToNextYear(paramValues);
	} else if (commandType.equals("COPYJIKANWARITONEXTYEAR")) {
	  handlerMethods.copyJikanwariToNextYear(paramValues);
	} else if (commandType.equals("COPYJIKANWARIOVERLAPTONEXTYEAR")) {
	  handlerMethods.copyJikanwariOverlapToNextYear(paramValues);
	} else if (commandType.equals("COPYGRADYOKENTONEXTYEAR")) {
	  handlerMethods.copyGradYokenToNextYear(paramValues);
	} else if (commandType.equals("COPYEDUYOKENTONEXTYEAR")) {
	  handlerMethods.copyEduYokenToNextYear(paramValues);
	} else if (commandType.equals("COPYMODULEDEFTONEXTYEAR")) {
	  handlerMethods.copyModuleDefToNextYear(paramValues);	
	} else if (commandType.equals("COPYIIFCURRICULUMTONEXTYEAR")) {
	  handlerMethods.copyIIFCurriculumToNextYear(paramValues);	
	} else if (commandType.equals("COPYYOMIKAETONEXTYEAR")) {
	  handlerMethods.copyYomikaeToNextYear(paramValues);	
	} else if (commandType.equals("SERVER_KILLER")) {
	  if ((commandCode.equals("MASTER_SERVER")) && 
