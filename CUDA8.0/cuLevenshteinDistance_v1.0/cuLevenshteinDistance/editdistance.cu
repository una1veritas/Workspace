...
#define MAX_THREADSPERBLOCK 128
...
long cu_lvdist(long * table, const char t[], const long n, const char p[], const long m) {
	...
	cu_dptable_init <<< grids, blocks >>> (devtable, devt, n, devp, m);
	// upper-left triangle
	dix = 1;
	for (dcol = 2; dcol <= m + 1; ++dcol) {
		dix += dcol;
		cu_dptable_topleft<<< grids, blocks >>> (devtable, devt, n, devp, m, dix, dcol);
	}
	// skewed rectangle
	for (dcol = m + 2; dcol < n + 1; ++dcol) {
		dix += m + 1;
		cu_dptable_center <<<  grids, blocks >>> (devtable, devt, n, devp, m, dix, dcol);
	}
	// bottom-right triangle
	for (dcol = n + 1; dcol < n + m + 2; ++dcol) {
		dix += n + m + 1 - dcol;
		cu_dptable_bottomright <<<  grids, blocks >>> (devtable, devt, n, devp, m, dix, dcol);
	}
