...
//int     override;
{
	...
	char tmpres[40] = "";
	if (!alpha(*str)) {
		sprintf(tmpres, "Illegal Symbol Name: '%.12s'", str);
		error(tmpres); //"Illegal Symbol Name");
		return (NO);
	}
...
#endif
    np = (struct nlist*) malloc(sizeof(struct nlist)); //alloc(sizeof(struct nlist));
	if (np == (struct nlist*) ERR) {
		error("Symbol table full");
		return (NO);
	}
    np->name = (char*) malloc(strlen(str) + 1); //alloc(strlen(str) + 1);
	if (np->name == (char*) ERR) {
		error("Symbol table full");
		return (NO);
	}
	...
	np->Lnext = NULL;
	np->Rnext = NULL;
    lp = (struct link*) malloc(sizeof(struct link)); //alloc(sizeof(struct link));
	np->L_list = lp;
	lp->L_num = Line_num;
	lp->next = NULL;
	p = root;
