...
static int load_config(void)
{
 ...
 fr = f_open(&fl, conf, 0x01);
 if ( fr != FR_OK ) {
  printf("%s not found..\r\n", conf);
  return -1;
 }

 while ( f_gets(str, 80, &fl) ) {
   ...
   *(t_conf[i].val) = (uint16_t)(long)__strtoxl((const char *)buf,&a,0,1);
  }
 }

 if (!((int)((&fl)->fptr == (&fl)->obj.objsize))) {
...
static int setup_msdos(void) {
 ...
 if (!flg) {
  ...
  if (!flg) {
   adr32 += ((((uint32_t)p_size+16) / 16)+1)*16;
...
static int load_program(uint8_t *fname, uint32_t load_adr) {
 ...
 fr = f_open(&fl, buf, 0x01);
 if ( fr != FR_OK ) return((int)fr);

 adr = load_adr;
 cnt = p_size = (UINT)((&fl)->obj.objsize);
 btr = 256 * 2;
 while( cnt ) {
...
static int open_dskimg(int os) {
 ...
 for (drv = num_files = 0; drv < 4 && num_files < 4; drv++) {
