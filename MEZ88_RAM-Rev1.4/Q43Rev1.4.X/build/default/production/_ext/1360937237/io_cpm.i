...
static int setup_drive(void) {
 req_tbl.CBI_CHR = 0;
 if ( req_tbl.disk_drive >= 4 ) return( -1 );
...
static uint8_t wr_time() {
 ...
 if (time_dev) {
  ...
  if (write_I2C(0b11010000, 0, 7, &rtc[0] ) == 0xFF) return 1;
 }
 else {
  TMR0IE = 0;
  read_sram( trans_adr, (uint8_t *)&tim_pb, sizeof(tim_pb) );
...
void cpm_bus_master_operation(void) {
 ...
 read_sram(0x7f500, (uint8_t *)&req_tbl, (unsigned int)sizeof(cpm_hdr));

 if (req_tbl.UREQ_COM) {
  unimon_console();

  write_sram(0x7f500, (uint8_t *)&req_tbl, 2);
 }
 else {
  ...
  write_sram(0x7f506, (uint8_t *)&req_tbl.CREQ_COM, 2);
 }
