...
void sdlchar(char ch) {
  ...
  if (echo_immediate) SDL_UpdateRect(screen0, xtext * XPPC, ytext * YPPC, XPPC, YPPC);
}
...
static void toggle_cursor(void) {
  ...
  if (echo_immediate) SDL_UpdateRect(screen0, xoffset + xtext*xscale*XPPC, yoffset + ytext*yscale*YPPC, xscale*XPPC, yscale*YPPC);
}
...
static void write_char(int32 ch) {
  ...
  xtext++;
  if (xtext > twinright) {
    if (!echo_immediate) echo_text();	/* Line is full so flush buffered characters */
    xtext = twinleft;
    ytext++;
  ...
  if (vduneeded == 0) {			/* VDU queue is empty */
    if (charvalue >= ' ') {		/* Most common case - print something */
      ...
      if (graphmode==FULLSCREEN) {	/* Flush any buffered text to the screen */
        if (!echo_immediate) echo_text();
      }
      else {
        if (!echo_immediate) echo_ttext();
      }
      vducmd = charvalue;
...
/* There are now enough entries in the queue for the current command */
  ...
  case VDU_BEEP:	/* 7 - Sound the bell */
    putchar('\7');
    if (echo_immediate) fflush(stdout);
    break;
