...
#define CMD_BUFFER_SIZE 8192	// size of working buffer (on heap) with extended RAM
#else
...
static FIL File[_FS_SHARE];			/* File object */
...
extern HTTP_REQUEST *pHTTPRequest;	// HTTPD request buffer - declared in webserver.c
									// Pointer to HTTP request

extern uint32_t EEMEM pagesServed;		// non-volatile storage for total pages served.
...
//	avrSerialPrintf_P(PSTR("brkval addr:            %x\r\n"),  (int16_t) __brkval);
    ...
    xTaskCreate(
		...
		,  384				// Tested ok at 188 for LED blink only.
		,  NULL
		,  3
		,  NULL ); // */
...
//		lcd_Printf_P(PSTR("Free Heap:%6u"),xPortGetFreeHeapSize() ); // needs heap_1 or heap_2 for this function to succeed.
		...
		lcd_Locate (1, 0);
		if( eeprom_is_ready() )
			lcd_Printf_P(PSTR("Pages:%u"), eeprom_read_dword(&pagesServed) ); // number of pages served by the http server.
