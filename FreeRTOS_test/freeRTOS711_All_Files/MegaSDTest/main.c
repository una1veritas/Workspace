...
#define BUFFER_SIZE 8192*3	// size of working buffer (on heap) with extended RAM
#else
#define BUFFER_SIZE 2048	// size of working buffer (on heap) for standard EtherMega
#endif

#define LINE_SIZE 128		// size of command line (on heap)

/*-----------------------------------------------------------*/
/* Create files for the  for the SD card and FatFS. */
...
static void TaskSDMonitor(void *pvParameters) // Monitor for SD Card
{
	...
	Buff = pvPortMalloc (sizeof(uint8_t) * BUFFER_SIZE);
	Line = pvPortMalloc (sizeof(uint8_t) * LINE_SIZE);
	...
	CurrTimeDate = pvPortMalloc (sizeof(xRTCArray) );
	SetTimeDate =  pvPortMalloc (sizeof(xRTCArraySto) );
    ...
    while(1)
    {
		...
		case 'b' :
			...
			case 'f' :	/* bf <n> - Fill working buffer */
				if (!xatoi(&ptr, &p1)) break;
				memset(Buff, (uint8_t)p1, (sizeof(uint8_t)* BUFFER_SIZE));
				break;
		...
		case 'f' :
			...
			case 'r' :	/* fr <len> - read file */
				...
				while (p1) {
					if (p1 >= (sizeof(uint8_t)* BUFFER_SIZE)) 	{ cnt = (sizeof(uint8_t)* BUFFER_SIZE); p1 -= (sizeof(uint8_t)* BUFFER_SIZE); }
			...
			case 'w' :	/* fw <len> <val> - write file */
				...
				memset(Buff, (uint8_t)p2, (sizeof(uint8_t)* BUFFER_SIZE));
				p2 = 0;
				Timer = xTaskGetTickCount();
				while (p1) {
					if (p1 >= (sizeof(uint8_t)* BUFFER_SIZE))	{ cnt = (sizeof(uint8_t)* BUFFER_SIZE); p1 -= (sizeof(uint8_t)* BUFFER_SIZE); }
			...
			case 'x' : /* fx <src_name> <dst_name> - Copy file */
				...
				*ptr2++ = 0;
				...
				p1 = 0;
				for (;;) {
					res = f_read(&File[0], Buff, (sizeof(uint8_t)* BUFFER_SIZE), &s1);
			...
			case 'q' :	/* fq - Show current dir path */
				res = f_getcwd(Line, (sizeof(uint8_t)* BUFFER_SIZE));
				if (res)
					put_rc(res);
				else
					xSerialPrintf_P(PSTR("%s\r\n"), Line);
...
static void TaskBlinkRedLED(void *pvParameters) // Main Red LED Flash
{
    ...
    while(1)
    {
		...
		vTaskDelayUntil( &xLastWakeTime, ( 450 / portTICK_RATE_MS ) );
