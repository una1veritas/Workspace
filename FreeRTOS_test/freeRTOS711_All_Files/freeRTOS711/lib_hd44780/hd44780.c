 ...
 */
#include <avr/iom2560.h> 			// xxx did this because PORTH #define couldn't be found.
...
#elif (defined(__AVR_ATmega328P__) || defined(__AVR_ATmega168__)) && defined(FreetronicsLCD)// assume we're using an Arduino with ATmega328p
...
//#define	DATA4(d) { if (d & _BV(4)) PORTD |= _BV(PD4); else PORTD &= ~_BV(PD4); } // normal Arduino Uno
...
#elif (defined(__AVR_ATmega328P__) || defined(__AVR_ATmega168__)) && defined(PololuLCD) // assume we're using an Arduino with ATmega328p
...
// LCD_DB6				PORTB5		// PB5
// LCD_DB5				PORTB4		// PB4
// LCD_DB4				PORTB3		// PB3 : Pololu was PB1, but I modified to avoid the Timer1 pins.
...
static uint8_t *LCDWorkBuffer; // create a working buffer pointer, to later be malloc() on the heap.
