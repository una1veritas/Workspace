...
uint32_t get_fattime (void)
{
...
#else
	// will do something smarter here when I can be bothered.
    struct
    {
            uint8_t Hour;
            uint8_t Minute;
    ...
    CurrTimeDate.Year   = (uint8_t)(2012 - 1980);
    CurrTimeDate.Month  = 2;
    CurrTimeDate.Day    = 4;
    ...
    return ((uint32_t)(CurrTimeDate.Year + 20) << 25) | // Microsoft: Y0 = 1980. DS1307: Y0 = 2000.
...
)
{
	...
	if (i == _FS_SHARE) {				/* Not opened. Register it as new. */
		for (i = 0; i < _FS_SHARE && Files[i].fs; i++) ;
		if (i == _FS_SHARE) return 0;	/* No space to register (int16_t err) */
		Files[i].fs = dj->fs;
		Files[i].clu = dj->sclust;
	...
	if (acc && Files[i].ctr) return 0;	/* Access violation (int16_t err) */
...
)
{
	...
	uint32_t nxt;
#if _USE_ERASE
	uint32_t scl = clst, ecl = clst, region[2];
#endif
	...
	if (clst < 2 || clst >= fs->n_fatent) {	/* Check range */
		...
		while (clst < fs->n_fatent) {			/* Not a last link? */
			...
			if (ecl + 1 == nxt) {	/* Next cluster is contiguous */
				...
				region[0] = clust2sect(fs, scl);					/* Start sector */
				region[1] = clust2sect(fs, ecl) + fs->csize - 1;	/* End sector */
				if ( disk_ioctl(fs->drv, CTRL_ERASE_SECTOR, region) != RES_OK)		/* Erase the block */
					res = FR_DISK_ERR;
				scl = ecl = nxt;
			}
#endif
...
)
{
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	stat = disk_initialize(fs->drv);	/* Initialise low level disk I/O layer */
	if (stat & STA_NOINIT)				/* Check if the initialisation succeeded */
		return FR_NOT_READY;			/* Failed to initialise due to no media or hard error */
