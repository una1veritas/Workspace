...
uint16_t	ff_convert (uint16_t, uint16_t);/* OEM-Unicode bidirectional conversion */
...
/* Sync functions */
#if _FS_REENTRANT
int16_t		ff_cre_syncobj (uint8_t, xSemaphoreHandle*);	/* Create a sync object */
int16_t		ff_req_grant (xSemaphoreHandle);				/* Lock sync object */
void		ff_rel_grant (xSemaphoreHandle);				/* Unlock sync object */
int16_t		ff_del_syncobj (xSemaphoreHandle);				/* Delete a sync object */
#endif
