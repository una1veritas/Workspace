...
void spiBegin(SPI_SLAVE_SELECT SS_pin)
{
	...
	// Set the control register to turn on the SPI interface.
	SPCR |= _BV(MSTR);
	SPCR |= _BV(SPE);
...
inline uint8_t spiSelect (SPI_SLAVE_SELECT SS_pin)	/* 1:Successful, 0:Timeout */
{
	...
	if ( (xSemaphoreTake(xSPISemaphore, (SPI_TIMEOUT  / portTICK_RATE_MS )) == pdTRUE)	/* FreeRTOS */)
	{
		...
		case SS_PB0:	// default SS line for Arduino Uno
		default:
			SPI_SS(0);
			break;
		...
		case SS_PB2:	// default SS line for Arduino Mega2560 (EtherMega)
			SPI_SS(0);
			break;
...
inline void spiDeselect (SPI_SLAVE_SELECT SS_pin)
{
	...
	case SS_PG5:	// added for the EtherMega SD Card support with SS on PG5
		// these 8 clock cycles are critical for a  MMC/SD card
		// to finish up whatever it's working on at the
		// time... (before SS is released!)
		spiTransfer(0xFF);

		SPI_SS_MEGA_SD(1);

		// To make a MMC/SDC card release DO signal, the master device
		// must send a byte after CS signal is deasserted.
		spiTransfer(0xFF);	/* Dummy clock (force DO hi-z for multiple slave SPI) */
